...
function buildPoints(v0: number, x0: number, duration: number, steps = 300) {
  const x: GraphPoint[] = []
  const v: GraphPoint[] = []
...
export default function MRU() {
  ...
  const xPos = x0 + v0 * currentTime
  const graphData = buildPoints(v0, x0, duration)
  ...
  const trackY = 90
  const xFinal = x0 + v0 * duration
  const xRange = Math.max(10, Math.abs(xFinal - x0) + Math.abs(x0) * 0.5 + 5)
  const xCenter = (x0 + xFinal) / 2
  const toSvgX = (x: number) => svgW / 2 + ((x - xCenter) / xRange) * svgW * 0.8
  const bodyX = Math.max(16, Math.min(svgW - 16, toSvgX(xPos)))
  const arrowLen = Math.min(90, Math.max(0, Math.abs(v0) * 16))
  const velDir = v0 >= 0 ? 1 : -1
      ...
      <div className="flex flex-1 min-h-0">
        ...
        <aside className="w-56 flex-shrink-0 border-r border-slate-800 bg-slate-900/50 p-4 overflow-y-auto">
          ...
          <div className="space-y-5">
            <SliderInput label="Velocità v₀" unit="m/s" min={-20} max={20} step={0.5} value={v0}
          ...
          <div className="mt-4 p-3 bg-slate-800/50 rounded-lg border border-slate-700/50">
            <p className="text-xs text-slate-500 font-mono">x(t) = x₀ + v₀·t</p>
          </div>
        </aside>
        ...
        <div className="flex-1 flex flex-col min-h-0">
          ...
          <div className="flex-1 bg-slate-950 flex items-center justify-center min-h-0 overflow-hidden p-2">
            <svg viewBox={`0 0 ${svgW} ${svgH}`} className="w-full h-full" style={{ maxHeight: 200 }} preserveAspectRatio="xMidYMid meet">
              ...
              <text x={svgW - 8} y={trackY + 15} fill="#64748b" fontSize="8">m</text>
